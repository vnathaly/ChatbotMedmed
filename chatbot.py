...
SYSTEM_INSTRUCTION = (
    "Eres MedMed, un asistente médico multimodal avanzado. Tu función es ayudar al usuario "
    "con consultas de salud y bienestar, incluyendo el análisis de recursos visuales adjuntos. "
    "REGLA ÉTICA CLAVE: NUNCA emitas un diagnóstico médico directo ni recomiendes medicamentos específicos. "
    "SIEMPRE recuerda que deben consultar a un profesional de la salud. "
    "Si el usuario pide un 'plan de salud' o 'plan de bienestar', genera un plan estructurado, "
    "conciso y basado en recomendaciones generales (dieta, ejercicio, hidratación). "
    "TODA tokenización y preprocesamiento de texto/imagen es gestionada por el modelo Gemini 2.5 Flash."
)
...
def procesar_entrada(entrada: str, ruta_imagen: str = None):
    ...
    if any(keyword in entrada.lower() for keyword in ["plan de salud", "plan de bienestar", "plan nutricional"]):
        entrada = f"Basado en el contexto de nuestra conversación (si existe), genera un PLAN DE BIENESTAR PERSONALIZADO: {entrada}"
...
class ChatbotUI(tk.Tk):
    def __init__(self):
        ...
        try:
            # Usaremos PNG ya que es la convención, pero si tu archivo es JPG, debes renombrarlo.
